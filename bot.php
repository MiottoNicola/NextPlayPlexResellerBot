...
class Bot {
  ...
  public function __construct($token,$json = false){
    ...
    if(isset($this->update['callback_query']['id'])){ //Se l'update è un callback_query
      ...
      $this->callback_cognome = $this->update['callback_query']['from']['last_name']; //nome dell'utente che ha cliccato il pulsante
      $this->callback_username = $this->update['callback_query']['from']['username']; //username dell'utente che ha cliccato il pulsante
      ...
      if(isset($this->update[$this->messageType]['new_chat_participant'])){ //Se c'è un nuovo partecipante alla chat
        ...
        $this->nuovo_membro_nome = $this->update['new_chat_member']['first_name']; //Nuovo membro
        ...
        $this->nuovo_partecipante_is_bot= $this->update[$this->messageType]['new_chat_participant']['is_bot'];
        $this->nuovo_partecipante_id = $this->update[$this->messageType]['new_chat_participant']['id'];
        $this->nuovo_partecipante_nome = $this->update[$this->messageType]['new_chat_participant']['first_name'];
      ...
      if(isset($this->update[$this->messageType]['audio'])){ //Se è una audio
        $this->durata_audio = $this->update[$this->messageType]['audio']['duration']; //Durata audio
        $this->audio_id = $this->update[$this->messageType]['audio']['file_id']; //File_id del audio
        $this->tipo_audio = $this->update[$this->messageType]['audio']['mime_type']; //Estensione del audio
        $this->width_audio = $this->update[$this->messageType]['audio']['width'];
        $this->size_audio = $this->update[$this->messageType]['audio']['file_size'];
        $this->height_audio = $this->update[$this->messageType]['audio']['height'];
      } 
      if(isset($this->update[$this->messageType]['voice'])){ //Se è una voice
        $this->durata_voice = $this->update[$this->messageType]['voice']['duration']; //Durata voice
        $this->voice_id = $this->update[$this->messageType]['voice']['file_id']; //File_id del voice
        $this->tipo_voice = $this->update[$this->messageType]['voice']['mime_type']; //Estensione del voice
        $this->width_voice = $this->update[$this->messageType]['voice']['width'];
        $this->size_voice = $this->update[$this->messageType]['voice']['file_size'];
        $this->height_voice = $this->update[$this->messageType]['voice']['height'];
      }     
      ...
      if(isset($this->update[$this->messageType]['reply_to_message']['from']['last_name'])){ //Verifica se l'utente ha il cognome
        ...
        $this->reply_to_message_photo_0_file_id = $this->update[$this->messageType]['reply_to_message']['photo'][0]['file_id'];
  ...
  public function editMessageText2($user_id, $message_id, $newText, $keyboard = false, $type = false, $parse_mode = false, $disableWebPagePreview = true){
	...
	public function editMessageCaption($chat_id, $message_id, $caption, $keyboard = false, $type = false, $parse_mode = 'HTML'){
	  ...
	  $url = 'https://api.telegram.org/bot'.$this->bot."/editMessageCaption?chat_id=$chat_id&message_id=$message_id&caption=$caption&reply_markup=$rm&parse_mode=$parse_mode";
  ...
  public function sendInvoice($provider_token, $chat_id, $title, $description, $payload, $prices, $currency = EUR){
    $prices = json_encode($prices);
    $url = 'https://api.telegram.org/bot'.$this->bot."/sendInvoice?chat_id=$chat_id&title=".urlencode($title)."&description=".urlencode($description)."&payload=$payload&provider_token=$provider_token&currency=$currency&prices=[$prices]";
    return $this->cURL($url);
  }

  public function answerPreCheckoutQuery($pre_checkout_query_id, $ok){
    $url = 'https://api.telegram.org/bot'.$this->bot."/answerPreCheckoutQuery?pre_checkout_query_id=$pre_checkout_query_id&ok=$ok";
    return $this->cURL($url);
  }

  public function sendDice($user_id, $emoji, $keyboard=false){
    ...
    $url = 'https://api.telegram.org/bot'.$this->bot."/sendDice?chat_id=$user_id&emoji=".urlencode($emoji).$rm;
    return $this->cURL($url);
  }
