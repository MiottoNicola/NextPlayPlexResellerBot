...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST['action'] == 'editAccount' && isset($_POST['id'])) {
        ...
        $embyID = (int) $_POST['embyID'];
        $email = $db->real_escape_string($_POST['mail']);
        $expiration = date('Y-m-d', strtotime($_POST['expiration']));
        $stmt = $db->prepare("UPDATE $tableClientsEmby SET expiration = ? WHERE ID = ?");
        ...
        $stmtClient = $db->prepare("SELECT embyID FROM $tableClientsEmby WHERE ID = ?");
        $stmtClient->bind_param("i", $id);
        $stmtClient->execute();
        $resultClient = $stmtClient->get_result();
        $stmtClient->close();

        $response = delete_user($resultClient->fetch_assoc()['embyID']);
        if($response) {
            $stmt = $db->prepare("DELETE FROM $tableClientsEmby WHERE ID = ?");
...
<h2 class="w3-text-teal" style="margin:40px 0 4px 0; text-align:center;">Gestione Account Emby</h2>
<?php
$resCount = $db->query("SELECT COUNT(*) as total FROM $tableClientsEmby");
...
<table class="w3-table-all w3-hoverable w3-centered"
    ...
    <tbody>
        <?php
        $stmt = $db->prepare("SELECT * FROM $tableClientsEmby ORDER BY ID ASC");
        $stmt->execute();
        ...
        if ($result && $result->num_rows > 0):
            while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($row['embyID']); ?></td>
                    <td><?php echo htmlspecialchars($row['mail']); ?></td>
                    <td><?php echo invertiData($row['expiration']); ?></td>

                    <td>
                        <a href="#"
                            onclick="openAccountModal('<?php echo $row['ID']; ?>','<?php echo $row['user_id'] ?>','<?php echo $row['embyID']; ?>','<?php echo $row['mail']; ?>','<?php echo $row['expiration']; ?>');return false;"
            ...
            <?php endwhile; else: ?>
            <tr>
                <td colspan="6">Nessun account emby trovato.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
...
<div id="editAccountModal" class="w3-modal">
    <div class="w3-modal-content w3-animate-top w3-card-4" style="max-width:440px;">
        <header class="w3-container">
            ...
            <h3 style="margin:0; font-size:1.35em; letter-spacing:0.5px;">Modifica Account Emby</h3>
        </header>
        <form class="w3-container" method="post" action="#">
            ...
            <div class="w3-section">
                <label class="w3-text-grey" style="font-weight:500;">EmbyID</label>
                <input class="w3-input w3-border w3-margin-bottom" type="text" name="emby_id" id="editEmbyId" readonly
                    style="background:#f5f5f5;">
            </div>
            <div class="w3-section">
...
<script>
    function openAccountModal(id, user_id, emby_id, email, expiration) {
        document.getElementById('editId').value = id;
        document.getElementById('editUserId').value = user_id;
        document.getElementById('editEmbyId').value = emby_id;
