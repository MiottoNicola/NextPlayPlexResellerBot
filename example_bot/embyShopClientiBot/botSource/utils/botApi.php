...
class Bot{
  ...
  public function __construct($token, $json = false){
    ...
    if($this->update != null){
      ...
      $getMe = $this->cURL('https://api.telegram.org/bot'.$this->bot.'/getMe');
      if($getMe['ok'] == true){
        $this->bot_id          = $getMe['result']['id'];            //ID del bot
        $this->bot_nome        = $getMe['result']['first_name'];    //nome del bot              
        $this->bot_username    = $getMe['result']['username'];      //username del bot                            
      }
  ...
  public function sendMessage($chat_id, $text, $keyboard = '', $reply_message_id = false){
    ...
    $url = 'https://api.telegram.org/bot'.$this->bot."/sendMessage?chat_id=$chat_id&text=" . urlencode($text) . '&parse_mode=HTML&disable_web_page_preview=true' . $keyboard . $reply_message_id . '&force_reply=false&disable_notification=false';
    return $this->cURL($url);
  }

  public function deleteMessage($chat_id, $message_id){
    $url = 'https://api.telegram.org/bot'.$this->bot."/deleteMessage?chat_id=$chat_id&message_id=$message_id";
    return $this->cURL($url);
  }
  ...
  public function editMessageText($chat_id, $message_id, $text, $keyboard = ''){
    ...
    $url = 'https://api.telegram.org/bot'.$this->bot."/editMessageText?chat_id=$chat_id&message_id=$message_id&text=".urlencode($text) . '&disable_web_page_preview=true&parse_mode=HTML'. $keyboard;
    return $this->cURL($url);
  }

  public function forwardMessage($from_chat_id, $chat_id, $message_id){
    $url = 'https://api.telegram.org/bot'.$this->bot."/forwardMessage?from_chat_id=$from_chat_id&chat_id=$chat_id&message_id=$message_id";
    return $this->cURL($url);
  }
 
  public function sendChatAction($chat_id, $action = 'typing'){
    $url = 'https://api.telegram.org/bot'.$this->bot."/sendChatAction?chat_id=$chat_id&action=$action";
    return $this->cURL($url);
  }
 
  public function sendSticker($chat_id, $sticker){
    $url = 'https://api.telegram.org/bot'.$this->bot."/sendSticker?chat_id=$chat_id&sticker=$sticker";
    return $this->cURL($url);
  }
  ...
  public function answerCallbackQuery($callback_query_id, $text){
    $url = 'https://api.telegram.org/bot'.$this->bot."/answerCallbackQuery?callback_query_id=$callback_query_id&text=".urlencode($text).'&show_alert=true';
    return $this->cURL($url);
  }
  ...
  public function sendInvoice($chat_id, $title, $description, $payload, $prices, $provider_token='', $currency = 'XTR'){
    ...
    $url = 'https://api.telegram.org/bot'.$this->bot."/sendInvoice?chat_id=$chat_id&title=".urlencode($title)."&description=".urlencode($description)."&payload=$payload".$provider_token."&currency=$currency&prices=[$prices]";
    return $this->cURL($url);
  }
...
class BotUtils {
  function setWebhook($token, $url, $max_connections = 40, $secret = 'EmbyShopBot2025'){
    $url = 'https://api.telegram.org/bot'.$token.'/setWebhook?max_connections='.$max_connections.'&secret_token='.$secret.'&drop_pending_updates=true&url='.$url;
    return $this->cURL($url);
  }
...
//https://api.telegram.org/bot7992035052:AAGbyj7qqmD5AFJcDYsdOZksAZjX1F6Xgq8/setWebhook?url=https://telegram.sonicmaster.it/embyShopBot/index.php?token=7992035052:AAGbyj7qqmD5AFJcDYsdOZksAZjX1F6Xgq8&max_connections=40&drop_pending_updates=True&secret_token=s7BuFjh72N86Oaq8WsIyI